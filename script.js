...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const showError = (message) => {
        alert(message); // Simple alert, can be replaced with a custom notification
    };
    
    // --- PRODUCT LOGIC ---
    const isProductAvailable = (product) => {
        if (product.sizes && product.sizes.length > 0) {
            // Stitched: available if at least one size has quantity > 0
            return product.sizes.some(s => s.quantity > 0);
        } else {
            // Unstitched/Skincare: available if stock > 0
    ...
    const openCheckoutModal = () => {
        ...
        checkoutBody.innerHTML = `
            <div class="order-summary">
                ...
                ${cart.map(item => `
                    <p>${item.name} ${item.size ? `(${item.size})` : ''} x ${item.quantity} = ৳${item.price * item.quantity}</p>
                `).join('')}
                </div>
                <hr>
            ...
            <form id="checkout-form">
                ...
                <div class="form-group">
                    ...
                    <input type="tel" id="customer-phone" required pattern="[0-9]{11}" title="Please enter a valid 11-digit phone number.">
                </div>
                <div class="form-group">
                ...
                <div class="form-group">
                    <label for="delivery-zone">Delivery Zone</label>
                    <select id="delivery-zone" required>
                        <option value="">-- Select Zone --</option>
                        ${deliveryCharges.map(d => `<option value="${d.charge}">${d.zone} - ৳${d.charge}</option>`).join('')}
                    </select>
                </div>
    ...
    const handleOrderSubmit = async (e) => {
        ...
        try {
            ...
            if (result.status === 'success') {
                ...
                saveCart();
                closeCheckoutModal();
                // Optionally reload data to reflect stock changes
                fetchProducts();
            } else {
