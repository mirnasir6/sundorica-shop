document.addEventListener('DOMContentLoaded', () => {
    ...
    const setupEventListeners = () => {
        ...
        checkoutModal.querySelector('#close-checkout').onclick = closeCheckoutModal;
    ...
    const fetchProducts = async () => {
        try {
            ...
            if (result.status === 'success') {
                ...
                showError('Failed to load products: ' + (result.message || 'Unknown error'));
            }
        } catch (error) {
        ...
        modalBody.innerHTML = `
            <div class="modal-images">
                ...
                <div class="thumbnail-images">
                    ${product.images.map((img, index) => `<img src="${img}" class="thumbnail ${index === 0 ? 'active' : ''}" data-src="${img}">`).join('')}
                </div>
            </div>
            <div class="modal-details">
                <h2>${product.name}</h2>
                <p class="product-price">৳${product.price} ${product.compareAtPrice ? `<span class="compare-price">৳${product.compareAtPrice}</span>` : ''}</p>
                ${hasSizes ? `
                <div class="size-options">
                    <label>Select Size:</label>
                    <div id="size-buttons">${product.sizes.map(s => `<button class="size-btn" data-size="${s.size}" ${s.quantity === 0 ? 'disabled' : ''}>${s.size}</button>`).join('')}</div>
                </div>` : ''}
                <div class="quantity-control"><label>Quantity:</label><input type="number" id="quantity-input" value="1" min="1"></div>
    ...
    const handleAddToCart = (product) => {
        const hasSizes = product.sizes && product.sizes.length > 0;
        const selectedSizeEl = productModal.querySelector('.size-btn.selected');
        const size = hasSizes ? selectedSizeEl?.dataset.size : null;
        if (hasSizes && !size) {
            showError('Please select a size.');
            return;
        }
        const quantity = parseInt(productModal.querySelector('#quantity-input').value);
    ...
    const addToCart = (item) => {
        const cartItem = { sku: item.sku, name: item.name, price: item.price, image: item.images[0], size: item.size, quantity: item.quantity };
    ...
    const openCheckoutModal = () => {
        const checkoutBody = checkoutModal.querySelector('#checkout-body');
        if (cart.length === 0) {
        ...
        checkoutBody.innerHTML = `
            <div class="order-summary">
                <h4>Order Summary</h4>
                <div id="cart-items-summary">${cart.map(item => `<p>${item.name} ${item.size ? `(${item.size})` : ''} x ${item.quantity} = ৳${item.price * item.quantity}</p>`).join('')}</div>
                <hr>
        ...
        const zoneSelect = checkoutModal.querySelector('#delivery-zone');
        zoneSelect.onchange = () => {
        ...
        checkoutModal.querySelector('#checkout-form').onsubmit = handleOrderSubmit;
    };
    ...
    const handleOrderSubmit = async (e) => {
        ...
        const form = e.target;
        const deliveryCharge = parseFloat(form.querySelector('#delivery-zone').value);
        ...
        const orderPayload = {
            ...
            totalQuantity, productPrice, deliveryCharge, totalAmount: productPrice + deliveryCharge
        };
